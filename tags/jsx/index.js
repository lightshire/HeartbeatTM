(function(htbt, React, toastr){
		...
		render: function(){
	    	return (
	    		<div className='inline_edit_cell' onDoubleClick={this.show_edit}>
	    			<label ref='lbl' className='inline_edit_label'  style={{display: 'block'}}>{this.state.data}</label>
	    			<input ref='txt' className='inline_edit_control' type='text' onBlur={this.handle_blur} onChange={this.save_value} style={{display: 'none'}} />
	    		</div>
	    	);
	  	},
	...
	React.render(<GridContainer />, document.getElementById("grid_container"))
